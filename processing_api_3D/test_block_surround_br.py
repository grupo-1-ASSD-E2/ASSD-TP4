...
conv = Convolutioner(IRs_left, IRs_right)
br = np.load('processing_api_3D/tests output/Bohemian Rhapsody ndarray.npy')
conv.set_input(br, tracks_dtype=conv.input_array.dtype)

conv.start_blocking_processing()

conv.save_output_array('processing_api_3D/tests output/surround_br_ndarray.npy')
conv.get_output_file('processing_api_3D/tests output/surround_br.wav')